...
> `Trie` is a data structure which is very similar to tree.we use `Trie` data structure when we want to store the list of string. so we can find any pattern of string using trie data structure.
